...
bool Parse_data(uint8_t *Data, char Offset, WiFiUDP &Udp) {
  ...
  // 1. パケットを受信したかどうかの判定
  if (packetSize) {
    *Data = Udp.read(); // データを1Byte読み込む
    
    // 2. 受信データが40以上であればBPM値と判定（フラグONを返す）
    if (*Data >= 40) {
      Serial.print("【受信】BPM値パケットを受信しました：");
      Serial.println(*Data);
      return true; // フラグON
    }
    // 3. 40未満であれば小節番号と判定（オフセットを適用し，フラグOFFを返す）
    else {
      Serial.print("【受信】サーバーの小節番号：");
      Serial.print(*Data);
      
      // 楽器ごとのオフセット値を適用して輪唱位置を調節
      *Data = *Data + Offset; 
      
      Serial.print(" -> オフセット補正後のマイ小節番号：");
      Serial.println((int)*Data);
      return false; // フラグOFF
    }
  }
  else {
    // パケットを受信していない場合もフラグOFFを返す設計
    return false;
  }
}
