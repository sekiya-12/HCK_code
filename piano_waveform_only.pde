...
// 波形だけを表示
// ==================================================
void drawWaveformOnly() {
  if (out == null || out.mix == null || out.mix.size() <= 1) {
    return;
  }
  ...
  int waveX = margin;
  int waveY = margin;
  int waveW = width - margin * 2;
  int waveH = height - margin * 2;
  int centerY = waveY + waveH / 2;

  // 中心線だけ薄く表示
  stroke(180);
  strokeWeight(1);
  line(waveX, centerY, waveX + waveW, centerY);
  ...
  for (int i = 0; i < out.mix.size(); i++) {
    float sample = out.mix.get(i);

    float x = map(i, 0, out.mix.size() - 1, waveX, waveX + waveW);
    float y = map(sample, -1.0, 1.0, waveY + waveH, waveY);
