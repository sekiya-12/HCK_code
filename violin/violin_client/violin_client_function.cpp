  ...
  if (millis() - *StartTime >= *Interval) {
    ...
    if (*NoteIndex < Score[*CurrentBar].noteCount) {
      ...
      *StartTime = millis(); // 次の音の基準時間を現在時刻に更新
      ...
      // ★修正：譜面配列から「周波数」「音の長さ」「弾き方」の3つを取得する
      float freq = Score[*CurrentBar].notes[*NoteIndex].freq;
      float duration = Score[*CurrentBar].notes[*NoteIndex].duration;
      int articulation = Score[*CurrentBar].notes[*NoteIndex].articulation;
      
      // 次の音までの間隔（Interval）を計算（ミリ秒）
      *Interval = ToneLength * duration;
      ...
      Serial.print(",");
      Serial.println(articulation); // ★追加：3つ目のデータとして弾き方を送信
